...
//-----------------------------------------------------------------------------------------------------------
    ...
    /*Get Order Details using Server side Table*/
...
//-----------------------------------------------------------------------------------------------------------
    ...
    /*Get Pickup Order Details using Server side Table*/
        ...
        if($this->request->getData('id') != '' && $this->request->getData('status') != '') {
            ...
            if($orderSave) {
                ...
                if(!empty($orderDetails) && $orderDetails['user']['device_id'] != '') {
                    ...
                    $this->PushNotification->customerNotification($orderDetails['customer_id'],'',$customerMessage);
                ...
                if(!empty($orderDetails) && $orderDetails['driver']['device_id'] != '') {
                    ...
                    $gcm = (trim($orderDetails['driver']['device_name']) == 'ANDROID') ?
                        ...
                        $this->IosNotification->notificationIOS($driverMessage, $orderDetails['driver']['device_id'], 'Customer');
...
//-----------------------------------------------------------------------------------------------------------
    ...
    public function ajaxaction() {
                ...
                if($orderDetails['driver_id'] == 0) {
                    $driverLists = $this->Drivers->find('all', [
                        'conditions' => [
                            ...
                            'Drivers.created_id' => $this->Auth->User('id'),
                    ...
                    if(!empty($driverLists)) {
                        ...
                        foreach ($driverLists as $dKey => $dValue) {
                            if($dValue['driver_tracking']['driver_latitude'] != '' && $dValue['driver_tracking']['driver_longitude'] != '') {
                                ...
                                $distance = $this->Googlemap->getDrivingDistance($dValue['driver_tracking']['driver_latitude'],                $dValue['driver_tracking']['driver_longitude'],$orderDetails['source_latitude'],$orderDetails['source_longitude']);
        ...
        if($this->request->getData('action') == 'LoadAllMap') {
            ...
            // echo "<pre>";print_r($ordersList);echo "</pre>";
            $action = $this->request->getData('action');
            $this->set('action', $action);
        ...
        if($this->request->getData('action') == 'dispatchUpdate') {
            ...
            $waitingList  = $this->Order->find('all', array(
                'conditions' => array(
                    'Order.order_type' => 'Delivery',
                    'Order.created LIKE' => $currentDate.'%',
                    'OR' => array('Order.status' => 'Waiting')),
                'fields' => array(
                    'Order.id','Order.store_id','Order.status','Order.ref_number',
                    'Order.payment_type','Order.customer_name','Order.assoonas',
                    'Order.delivery_date','Order.delivery_time','Order.google_address'),
                'contain' => array(
                    'Store' => array(
                        'fields' => array('Store.store_name')),
                    'Driver' => array(
                        'fields' => array('Driver.driver_name'))),
            ...
            $action = $this->request->getData('action');
            $this->set(compact('acceptedOrder','action'));
        }
    }
//-----------------------------------------------------------------------------------------------------------
        ...
        if (!$ordersDetails['driver_id']) {
            ...
            if (!empty($driverDetails)) {
                ...
                $gcm    = (trim($driverDetails['device_name']) == 'ANDROID') ?
                    ...
                    $this->IosNotification->notificationIOS($notificationdata['data']['message'], $deviceId, 'Driver', 'Driver', $orderDetails);
        ...
        if($this->request->getData('id') != '' && $this->request->getData('status') != '') {
            ...
            if($orderSave) {
                ...
                if(!empty($orderDetails) && $orderDetails['driver']['device_id'] != '') {
                    ...
                    $gcm = (trim($orderDetails['driver']['device_name']) == 'ANDROID') ?
                        $this->FcmNotification->sendNotification($notificationdata, $orderDetails['driver']['device_id']) :
                        $this->PushNotifications->notificationIOS($driverMessage, $orderDetails['driver']['device_id'], 'Customer');
...
//-----------------------------------------------------------------------------------------------------------
    ...
    /*Send Order Mail When accept a order*/
