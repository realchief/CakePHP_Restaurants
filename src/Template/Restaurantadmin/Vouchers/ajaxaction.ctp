<?php if($action == 'voucherStatuschange') { ?>
    <?php if($status == 'active'){?>        
            <button href="javascript:;" onclick="changeStatus('<?= $id ?>', '0', '<?= $field ?>', 'offers/ajaxaction', 'voucherStatuschange')"><i class="fa fa-check"></i>
            </button>        
    <?php }else { ?>       
            <button href="javascript:;" onclick="changeStatus('<?= $id ?>', '1', '<?= $field ?>', 'offers/ajaxaction', 'voucherStatuschange')"><i class="fa fa-close"></i>
            </button>        
    <?php } ?>
<?php die(); } ?>
            ...
            <?php if(!empty($vouchersList)) {
                foreach($vouchersList as $key => $value) { ?>
                    <tr>
                        ...
                        <td>
                            <a href="<?php echo ADMIN_BASE_URL ?>vouchers/edit/<?php echo $value['id'] ?>"><i class="fa fa-pencil"></i></a>
